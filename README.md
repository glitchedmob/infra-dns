# LZ DNS Infrastructure

OpenTofu configuration for LZ DNS management in Cloudflare.

## Scope

- OpenTofu in `src/tf/` manages DNS zones and records.
- v1 targets the `levizitting.com` zone and a narrow, explicit record set.

Core records in v1:

- `hello-nginx.levizitting.com` -> CNAME to `x86-vps-node-01.levizitting.com`
...
If `hello-nginx.levizitting.com` already exists in Cloudflare, import it before first apply:
